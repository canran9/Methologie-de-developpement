...
public class PlateauDeJeu {
 ...
 public void init() {
    ...
    public int nbGares(Joueur j) {
        ...
        for (Case c : plateau) {
            if (c instanceof Gare) {
                if (((Achetable) c).getProprietaire() == j) {
                    nbGare += 1;
                }
            }
        }
     ...
     *
     * @return si == 1 , c'est la fin de joueur
     */
    ...
    /**
     * Commerce une partie et fait tous les joueurs jusqu'à la fin de partie.
     */
    public void lancePartie() {
        System.out.println("Début de la parie !");
        while (!this.finPartie()) {
            this.tourDeJeu();
        }
        // Il ne reste qu'un joueur
        Joueur gagnant = this.joueurs.get(1);
        System.out.println("La partie est terminée.");
        System.out.println("Le gagnant est " + gagnant.getNom() + ".");
    }

    /**
     * Elimine le joueur er remet à zéro toutes ses propriétés.
     *
     * @param j Le joueur à eliminer
     */
